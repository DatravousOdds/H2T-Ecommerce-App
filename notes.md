...
**Bug found and fixed while verifying (round 1), not assumed away:** `.avatar-skeleton { border-radius: 50%; }` rendered as a square, not a circle. `skeleton.css`'s `.skeleton { border-radius: 4px; }` loads *after* `profile.css` in every page's `<head>`, so at equal selector specificity (0,1,0 each) it wins regardless of which class comes first in the element's `class` attribute. Caught via `getComputedStyle` in a Playwright check, not by eyeballing the screenshot — the rectangle was subtle enough at 100×100px to almost miss visually. Fixed by upping specificity with a combined selector (`.avatar-skeleton.skeleton { border-radius: 50%; }`), and applied the same fix preemptively to `.profile-background.skeleton`'s corner radius (8px top corners vs the shared class's flat 4px), same root cause, before it could cause the identical bug.

**First verification pass was insufficient — only checked `classList.contains("hidden")`, not what that class actually did.** Confirmed via a logged-out session (skeleton state never resolves, so it's easy to screenshot) and called it done. When later asked specifically to confirm the skeleton *disappears after a real load*, logged in as the seeded test account from `scripts_seed_chart_test_user.js` (`chart-debug-test@example.com` — checked into the repo for exactly this kind of debugging) and found two more real bugs that the class-presence check had missed entirely:

1. **The exact same cascade-order bug as the border-radius one, on `display` this time:** `.hidden { display: none }` (profile.css) was losing to `.skeleton { display: inline-block }` (skeleton.css, loads after) at equal specificity — so `.avatar-skeleton`/`.stat-skeleton` had `hidden` correctly *added* to their class list, but `getComputedStyle().display` stayed `inline-block`/`block`. The skeleton placeholders never actually disappeared after loading; they sat right next to the real content permanently. Fixed the same way as the border-radius bug: `.avatar-skeleton.hidden, .stat-skeleton.hidden { display: none; }`.
2. **Unrelated pre-existing bug, surfaced by the same login test:** the seeded test user has `profileImage: ""`. `loadProfileDisplayData()` did `#profile-picture.src = userData.profileImage` unconditionally — an empty string renders as a broken image (visible alt text overlapping the avatar circle) instead of falling back to the `/images/default-avatar.svg` the HTML already ships with before this code ever runs. One-line fix: `userData.profileImage || "/images/default-avatar.svg"`.

**Verified, properly this time:** logged in via Playwright as the seeded test account, navigated to `/profile`, captured computed styles (not just class lists) immediately after load (confirmed skeleton state: shimmering, real content hidden) and again once `loadProfileDisplayData()` had actually run (confirmed real username/join-date/follower-following-counts rendered, all four skeletons at `display: none`, default avatar showing cleanly). Screenshotted both states — no lingering placeholder artifacts in either.
...
## 2026-07-25 — Stripe Connect onboarding (prerequisite for delivery-confirmation payout hold)

**Context:** the delivery-confirmation feature being built (buyer confirms receipt before seller payout releases, dispute path for problems) requires a seller to be a destination money can move *to*. Nothing in the codebase let Hexxo pay a seller independently — checkout only ever captures the buyer's payment into Hexxo's own Stripe balance ([buildOrderDataFromPaymentIntent](server.js#L2098) / [handlePaymentIntentSucceeded](server.js#L2115)). This session built only that prerequisite — Connect onboarding itself, not the confirmation/dispute/payout-release logic that will eventually use it.

**Architecture decision — separate charges & transfers, not destination charges:** kept deliberately so checkout stays untouched. Money still lands in Hexxo's balance at checkout; a `stripe.transfers.create` to the seller only happens later, once a payout is actually approved (not built yet). This is also *why* a payout hold is possible at all — destination charges would split funds to the seller at checkout time, leaving nothing to withhold. Matches choosing **"Marketplace"** (not "Platform") in the Connect dashboard, and `defaults.responsibilities: { fees_collector: 'application', losses_collector: 'application' }` at [server.js:1690](server.js#L1690) — Hexxo, not the seller, owns the original charge, so Hexxo eats its fees/disputes.

**Forced off Accounts v1 mid-build:** Stripe now blocks `stripe.accounts.create({type: 'express'})` outright for new Connect integrations — not a deprecation warning, a hard stop hit by running the code. Migrated to Accounts v2 (`stripe.v2.core.accounts.create`, [server.js:1639](server.js#L1639)), which restructures capabilities under `configuration.recipient` instead of a flat `type`/`capabilities` shape. Confirmed live (not just assumed): v1's `stripe.accountLinks.create` ([server.js:1711](server.js#L1711)) works fine with a v2 account id — Stripe's documented v1/v2 interop held up in practice. Also found only via a live 400: `identity: { country: 'us' }` ([server.js:1672](server.js#L1672)) must be set before `defaults.currency` can be — undocumented dependency between two supposedly-independent fields.

**No webhook — deliberately deferred, not overlooked:** v2 accounts fire "thin events" (`v2.core.account.updated`), not the classic `account.updated` the existing webhook switch already handles ([server.js:104](server.js#L104)). Thin events carry no object inline, need a different verification method (`parseEventNotification`, not `constructEvent`), and require a separate Stripe Event Destination with its own secret — real new integration surface for what this session needed. `GET /api/connect/status` ([server.js:1733](server.js#L1733)) polls `stripe.v2.core.accounts.retrieve` on demand instead of receiving push updates. Known gap: status is only as fresh as the last poll — fine for a seller checking their own onboarding progress, not fine to trust for an unattended payout decision without re-fetching at that moment.

**Found and fixed while live-testing, not caught by reading docs:** `deriveConnectStatus()` ([server.js:1615](server.js#L1615)) first mapped Stripe's capability `status` field directly (`active`→complete, `restricted`→restricted, `pending`→pending). A real sandbox account, created *seconds* earlier with nothing submitted yet, came back `status: "restricted"` with `status_details.code: "requirements_past_due"` — Stripe overloads `"restricted"` to mean both "never started" and "actually blocked," so `status` alone can't tell a seller who hasn't opened the form yet from one under review. Fixed to read `status_details[].resolution` instead: `provide_info` (seller can fix it) → `pending`; `contact_stripe` (needs escalation) → `restricted`. Re-verified live: same untouched account now correctly reports `pending`.

**Verified, both branches, against real Stripe responses:** a test account (`test3@example.com`) completed Stripe's hosted onboarding using Stripe's own sandbox test values (SSN `000000000`, DOB `01/01/1902`, phone `0000000000`, address token `address_full_match`, bank routing `110000000`/account `000123456789`). `GET /api/connect/status` returned `{ connectOnboardingStatus: "complete", connectPayoutsEnabled: true }` afterward — confirms the `complete` branch, not just the `pending` one caught above.

**Not done / explicitly deferred:**
- **No frontend calls either endpoint yet.** `POST /api/connect/onboard` and `GET /api/connect/status` exist and are tested via curl with a real Firebase ID token, but nothing in `public/js` wires up a button or status display.
- **No gating on *who* triggers onboarding.** H2T has no fixed buyer/seller roles — any authenticated user can list an item, so this endpoint is per-account, not per-role. Testing used `test3@example.com`, an account that had only ever appeared as a *buyer* in existing order data — onboarding worked identically, since Stripe doesn't care what role an account has played before. In production this should naturally trigger the first time a user actually needs a payout (first listing created, or first approved delivery confirmation) rather than being gated by an explicit "is this a seller" check — that trigger point doesn't exist anywhere yet.
- **Thin-events/push updates** — still just polling, see above.
- **Local `.env` is running sandbox test keys** (`STRIPE_SECRET_KEY`, `STRIPE_ENDPOINT_SECRET`) for this work. Original live values backed up to `.env.bak` (untracked) — must restore before any deploy.

## 2026-07-26 — Seller Orders tab: `PUT /orders/:id` 401 on Mark Delivered after the tab had been open a while

**Symptom:** Clicking "Mark Delivered" (also reproducible on "Mark Shipped") threw `401 (Unauthorized)` from `PUT /orders/:id`, in [orders.js:359](public/js/pages/profile/selling/orders/orders.js#L359), for a user who was genuinely still logged in — confirmed by reloading the page and retrying successfully with no other change.

**Root cause:** `orders.js:12` does `const currentUser = await checkUserStatus();` at module load — a one-time snapshot, including `currentUser.idToken`, captured whenever the page first authenticates. `checkUserStatus()` itself ([auth.js:80-131](public/js/auth/auth.js#L80-L131)) caches its resolved user in a module-level `cachedUser` for the rest of the page's life and never re-fetches. Firebase ID tokens expire after ~1 hour; nothing in this chain ever calls `getIdToken()` again after that first snapshot, so `patchOrder()`/`cancelOrder()` kept sending that same string in the `Authorization` header indefinitely. Any tab left open longer than the token's lifetime 401s on its next authenticated action — not a login problem, a stale-token-string problem.

**Fix, scoped to this file:** `patchOrder()` and `cancelOrder()` in `orders.js` now call `await auth.currentUser.getIdToken()` fresh, immediately before each request, instead of reading `currentUser.idToken`. `getIdToken()` with no force flag is cheap either way — it returns the still-valid cached token instantly, or transparently refreshes it if expired — so there's no downside to calling it per-request instead of trusting an old snapshot. Needed importing the already-instantiated `auth` export from [firebase-client.js:52](public/js/api/firebase-client.js#L52) into `orders.js`.

**Known limitation, found but not fixed — same root cause, same fix would apply, out of scope of the reported bug:** the identical `currentUser.idToken`/`user.idToken` snapshot pattern also exists in `checkout.js`, `confirm.js`, `global.js`, `product.js`, `sellerProfileOffers.js`, and `purchases.js`. Any of them will 401 the same way on a long-lived tab. Worth the same one-line fix in each the next time one of them gets touched or reported — didn't do a sweep of all of them here since only the Orders-tab report was in scope tonight.

## 2026-07-27 — Seller "Set Up Payouts" card asked a seller to redo Stripe Connect onboarding they'd just finished

**Symptom:** A seller with no Connect account clicked "Set Up Payouts" on the Selling Overview tab, completed Stripe's entire hosted onboarding flow, got redirected back to `/profile?tab=selling` — and the same "Set Up Payouts" card was still there, as if nothing had happened. Only after going through the *whole flow a second time* did the card finally disappear.

**Root cause:** [overview.js](public/js/pages/profile/selling/overview/overview.js)'s `loadPayoutSetupCard()` calls `GET /api/connect/status` once, immediately on page load, and shows/hides the card based on whatever it gets back. Stripe doesn't flip a Connect account's capability status to `active` the instant the hosted flow's `return_url` redirects back — there's a real, brief propagation delay server-side. So the very first status check after finishing onboarding legitimately still came back `pending`, not because of a bug in the status derivation (that part was already verified correct earlier this session — see the [Stripe Connect onboarding entry](notes.md) above), but because the check ran before Stripe had caught up with its own hosted UI telling the seller they were done.

Confirmed by checking Firestore directly: `userProfiles/{sellerId}` already had `connectOnboardingStatus: "complete"` by the time this was investigated — the *second* onboarding round-trip is what finally caught a status check landing after the propagation delay had cleared. The first round-trip did everything right on Stripe's end; the app just asked too early.

**Fix:**
1. [server.js](server.js) — `/api/connect/onboard`'s `return_url`/`refresh_url` now include `&connect_return=1`, distinguishing "just came back from Stripe" from an ordinary page visit.
2. `loadPayoutSetupCard()` now retries the status check up to 3 times, 1.5s apart, but **only** when `connect_return=1` is present and the status comes back `pending` — a seller genuinely still mid-onboarding on a normal page visit doesn't pay this extra latency on every single load, since retrying can't make a real gap resolve on a timer anyway. The query param is stripped via `history.replaceState` right after being read, so a later manual refresh of the same URL doesn't re-trigger the retry-wait.

**Verified:** confirmed by the user against a brand-new seller test account with no prior Connect history -- completed hosted onboarding once, card correctly hid on the first return instead of asking for a second round-trip.
