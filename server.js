...
app.post('/login', (req, res) => {
    ...
    db.collection('users').doc(email).get()
        .then(user => {
            if (!user.exists) { // email does not exists
...
app.post('/seller', (req, res) => {
    ...
    if (!name.length || !addy.length || !about.length || num.length < 10 ||
        !Number(num)) {
...
app.post('/add-product', (req, res) => {
    let { name, shortDes, des, images, sizes, actualPrice, discount, sellerPrice,
    ...
    if (!draft) {
        if (!name.length) {
            ...
            return res.json({ 'alert': 'short description must be between 10 or 100 characters long' });
        } else if (!des.length) {
            return res.json({ 'alert': 'enter details description about the product' });
        } else if (!images.length) { //img link array 
            ...
            return res.json({ 'alert': 'enter few tags to help ranking your prodcut in search' });
        } else if (!tac) {
...
app.post('/get-products', (req, res) => {
    ...
    let docRf = id ? db.collection('products').doc(id) : db.collection('products').
        where('email', '==', email);
    ...
    db.collection('order').doc(docName).set(req.body)
        .then(data => {
            ...
            transporter.sendMail(mailOption, (err, info) =>{
                if(err){
                    res.json({'alert': 'opps! it looks like some error occured. Try again'})
                } else{
